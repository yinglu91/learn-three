function init() {
  ...
  const camera = initCamera(new THREE.Vector3(50, 80, 90));
  ...
  const controls = new (function () {
    ...
    this.addSpheres = function () {
      const colorSphere = colors[Math.floor(Math.random() * 5)];
      for (const i = 0; i < 5; i++) {
    ...
    this.addCubes = function () {
      const colorBox = colors[Math.floor(Math.random() * 5)];
      for (const i = 0; i < 5; i++) {
        box = new Physijs.BoxMesh(
          new THREE.BoxGeometry(4, 4, 4),
          Physijs.createMaterial(
            new THREE.MeshStandardMaterial({ color: colorBox }),
